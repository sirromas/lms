...
class Navigation extends Utils
{
    ...
    const ARTICLES_PATH = 'http://www.newsfactsandanalysis.com/articles';
    ...
    function get_article_url()
    {
        ...
        if ($mobile) {
            $articleURL="http://www." . $_SERVER['SERVER_NAME'] . "/articles/$path";
            $pdf = new mPDF('utf-8', 'A4-L');
            $list=file_get_contents($articleURL);
            $pdfPath=$_SERVER['DOCUMENT_ROOT']."/articles/$path.pdf";
            $pdf->WriteHTML($list, 2);
            $pdf->Output($pdfPath, 'F');
            //$url = "http://www." . $_SERVER['SERVER_NAME'] . "/articles/$path.pdf";
            // Temp workaround
            $url = "http://www." . $_SERVER['SERVER_NAME'] . "/articles/$path";
        } // end if
        else {
            $url = "http://www." . $_SERVER['SERVER_NAME'] . "/articles/$path";
        } // end else
